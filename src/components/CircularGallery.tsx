...
async function resolveFont(font, fontUrl) {
  if (!fontUrl) {
    if (document.fonts?.load) {
      try {
        ...
        // Fall back to the available system font.
      }
    }
    return font;
  }
  ...
  try {
    ...
    const urlMatch = cssText.match(/url\(\s*['"]?([^'")]+)['"]?\s*\)/);
    if (!familyMatch || !urlMatch) return font;
    const family = familyMatch[1].trim();
    const face = new FontFace(family, `url(${urlMatch[1]})`);
    await face.load();
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then((resolvedFont) => {
      ...
      runtime = new CircularGalleryRuntime(containerRef.current, {
        items,
        bend,
        textColor,
        borderRadius,
